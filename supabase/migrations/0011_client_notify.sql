-- "Notify client on low balance" preference.
alter table clients add column notify_low_balance boolean not null default true;

drop function if exists create_client(text, text, text, bigint, wallet_policy, currency_code, text, text, text, text, text);
...
begin
  ...
  insert into clients (company, contact, email, initials, threshold_paise, policy, currency,
                       mobile, address1, address2, city, state, notify_low_balance, balance_paise)
  values (trim(p_company), coalesce(p_contact, ''), lower(coalesce(p_email, '')),
          ...
          coalesce(p_city, ''), coalesce(p_state, ''), coalesce(p_notify, true), 0)
  returning id into cid;
  return cid;
end;
$$;
grant execute on function create_client(text, text, text, bigint, wallet_policy, currency_code, text, text, text, text, text, boolean) to authenticated;

drop function if exists update_client(uuid, text, text, text, bigint, wallet_policy, currency_code, text, text, text, text, text);
...
begin
  ...
  update clients set
    ...
    notify_low_balance = coalesce(p_notify, true)
  where id = p_id;
  if not found then raise exception 'client not found'; end if;
end;
$$;
grant execute on function update_client(uuid, text, text, text, bigint, wallet_policy, currency_code, text, text, text, text, text, boolean) to authenticated;
